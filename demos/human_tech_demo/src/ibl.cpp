...
float sky_fbm(float x,float y) {
  float v=0, a=0.55f;
  for(int i=0;i<5;++i) {v+=a*sky_noise(x,y); x=x*2.03f+17; y=y*2.03f-11; a*=0.48f;}
  return v;
}
}
...
Environment make_analytic_environment(Gpu& gpu, Vec3 sun_dir, std::uint32_t bg_size, std::uint32_t spec_size) {
  Equirect env;
  env.w = 1024; env.h = 512;
  env.rgb.resize(static_cast<std::size_t>(env.w)*env.h*3);
  const bool dusk = sun_dir.y < 0.15f;
  sun_dir=normalize(sun_dir);
  for(int y=0;y<env.h;++y) for(int x=0;x<env.w;++x) {
    const Vec3 d=env.dir_of(x,y);
    float up=std::max(d.y,0.0f);
    Vec3 sky=lerp(Vec3{1.08f,0.91f,0.72f},Vec3{0.19f,0.34f,0.49f},std::pow(up,0.4f));
    float dd=std::max(dot(d,sun_dir),0.0f);
    sky+=Vec3{1.0f,0.69f,0.36f}*(0.9f*std::pow(dd,12.0f));
    if(d.y>0) {
      // Clouds live on a fixed sky plane. Directional sampling has no seam.
      const float cx=d.x/(up+0.16f)*2.4f, cz=d.z/(up+0.16f)*2.4f;
      float cloud=clampf((sky_fbm(cx,cz)-0.44f)*4.0f,0,1);
      cloud*=clampf(up*10,0,1);
      const float edge=sky_fbm(cx+0.15f,cz-0.2f);
      Vec3 cloud_col=lerp(Vec3{0.34f,0.41f,0.48f},Vec3{1.55f,1.28f,0.96f},edge);
      sky=lerp(sky,cloud_col,cloud*0.87f);
    } else sky=Vec3{0.3f,0.29f,0.26f};
    if (dusk) sky=sky*0.14f*Vec3{0.48f,0.72f,1.0f};
    if(dd>std::cos(radians(0.65f))) sky=dusk?Vec3{450,240,120}:Vec3{26000,18000,10000};
    float* p=&env.rgb[(static_cast<std::size_t>(y)*env.w+x)*3];
    p[0]=sky.x;p[1]=sky.y;p[2]=sky.z;
  }
  Environment out=build_environment(gpu,std::move(env),0,bg_size,spec_size,false);
  if(dusk) {out.exposure=1.1f;out.has_sun=true;out.sun_dir=sun_dir;out.sun_color={0.4f,0.22f,0.1f};}
  return out;
}
