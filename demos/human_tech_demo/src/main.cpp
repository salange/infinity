// human_tech_demo — standalone city-block generator and renderer (tech faction).
...
Args parse(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!std::strcmp(argv[i], "--seed")) a.seed = next("--seed");
    ...
    else if (!std::strcmp(argv[i], "--shot")) a.shot = next("--shot");
      ...
      std::printf("human_tech_demo [--shot aerial|civic|street|terrace] [--seed S] [--width W --height H] [--hidden] [--capture out.png --frames N]\n"
                  "          [--sky studio|day|night|sunset|file.hdr] [--sky-yaw deg] [--night] [--assets dir]\n"
                  "          [--cam x,y,z --target x,y,z] [--msaa 1|4] [--debug 0-5] [--ev bias]\n");
      std::exit(0);
    } else {
  ...
  if (a.width < 64 || a.height < 64 || a.width > 8192 || a.height > 8192 || a.frames < 1 ||
      (a.msaa != 1 && a.msaa != 4)) {
    std::fprintf(stderr, "invalid dimensions, frame count or MSAA\n"); std::exit(2);
  }
  if (a.shot != "aerial" && a.shot != "civic" && a.shot != "street" && a.shot != "terrace") {
    std::fprintf(stderr, "--shot must be aerial, civic, street or terrace\n"); std::exit(2);
  }
  return a;
}
...
int main(int argc, char** argv) {
  ...
  GLFWwindow* window = glfwCreateWindow(args.width, args.height, "human_tech_demo", nullptr, nullptr);
  if (window == nullptr) {
  ...
  if (args.sky != "studio") env_day = cb::load_environment(gpu, sky_path(args.sky), yaw, 512, 128, true);
  if (!env_day.ok) {
    std::printf("  using the built-in late-afternoon sky\n");
    env_day = cb::make_analytic_environment(gpu, cb::Vec3{-0.62f, 0.48f, 0.62f}, 512, 128);
  }
  cb::Environment env_night = cb::load_environment(gpu, sky_path("night"), yaw, 512, 128, true);
  if (!env_night.ok) env_night = cb::make_analytic_environment(gpu, {-0.7f,0.10f,0.5f},512,128);
  ...
  if (!args.capture.empty()) {
    for (int i = 0; i < args.frames; ++i) renderer.render(cam, static_cast<float>(i) / 60.0f, nullptr);
    gpu.poll(true);
    ...
    renderer.shutdown();
    arrays.albedo.release(); arrays.normal.release(); arrays.arm.release();
    if (env_night.background.texture != env_day.background.texture) {
      env_night.background.release(); env_night.specular.release();
    }
    env_day.background.release(); env_day.specular.release();
    gpu.destroy(); glfwDestroyWindow(window); glfwTerminate();
    return ok ? 0 : 1;
  }
  double last = elapsed();
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (int i=0;i<4;++i) if (pressed(GLFW_KEY_1+i)) {
      static const char* shots[]={"aerial","civic","street","terrace"};
      cb::Vec3 target;
      cb::shot_camera(shots[i],cam.position,target);
      cam.look_at_point(target); cam.velocity={}; renderer.reset_history();
    }
    ...
    if (now - fps_t > 2.0) {
      char title[256];
      const cb::Renderer::Stats& rs = renderer.stats();
      std::snprintf(title, sizeof(title), "human_tech_demo — %.0f fps, %.1f M tris drawn of %.1f M, %u/%u ranges (%u occluded), %.0f m/s%s",
