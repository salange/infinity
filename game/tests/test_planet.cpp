...
TEST_CASE("planet params: ranges hold for 1000 seeds per type") {
  for (std::uint32_t type_index = 0; type_index < 4; ++type_index) {
    ...
    for (std::uint64_t seed = 0; seed < 1000; ++seed) {
      ...
      // Sea level is SOLVED from macro/v1 since T0015 WP1: on dry worlds
      // it sits below the global elevation minimum (negative), on wet
      // ones it is the quantile at (1 - land_fraction).
      REQUIRE(params.macro_amplitude_m.to_double() > 0.0);
      REQUIRE(params.macro_amplitude_m.to_double() < params.radius_m.to_double() * 0.01);
      if (type == gen::PlanetType::Barren) {
        REQUIRE(params.atmosphere_height_m.to_double() == 0.0);
        REQUIRE(params.land_fraction.to_double() == 1.0);
        REQUIRE(params.sea_level_m.to_double() < 0.0);
      }
      if (type == gen::PlanetType::EarthLike) {
        REQUIRE(params.land_fraction.to_double() >= 0.15);
        REQUIRE(params.land_fraction.to_double() <= 0.45);
        REQUIRE(std::abs(params.sea_level_m.to_double()) <
                params.macro_amplitude_m.to_double() * 4.0);
