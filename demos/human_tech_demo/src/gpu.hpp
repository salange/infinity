...
class GpuUnavailable : public std::runtime_error {
public:
...
public:
  ...
  bool submit(WGPUCommandBuffer command, bool wait);
  // Initialization/transport uploads only: submit pending writes and wait
  // before reusing the bounded staging allocation. Poll alone cannot do this.
  bool flush_uploads();
  [[noreturn]] void fail(const std::string &message);

  WGPUBuffer create_buffer(WGPUBufferUsage usage, std::uint64_t size,
                           const void *data, const char *label);
  void write_buffer(WGPUBuffer buffer, std::uint64_t offset, const void *data,
                    std::uint64_t size);
  void upload_buffer(WGPUBuffer buffer, std::uint64_t offset, const void *data,
                     std::uint64_t size);

  Texture create_texture(std::uint32_t w, std::uint32_t h,
                         WGPUTextureFormat format, WGPUTextureUsage usage,
                         std::uint32_t mips = 1, std::uint32_t layers = 1,
                         std::uint32_t samples = 1,
                         const char *label = "texture", bool cube = false);
  // Uploads one layer of RGBA8 texels and generates the mip chain on the CPU.
  void upload_rgba8_mips(const Texture &tex, std::uint32_t layer,
                         const std::uint8_t *rgba);
  // Uploads one layer/mip of RGBA16F (halfs) or RGBA32F data as-is.
  void upload_level(const Texture &tex, std::uint32_t layer, std::uint32_t mip,
                    std::uint32_t w, std::uint32_t h, const void *data,
                    std::uint32_t bytes_per_pixel);
  void upload_volume_level(const Texture &tex, std::uint32_t mip,
                            std::uint32_t w, std::uint32_t h,
                            std::uint32_t depth, const void *data,
                            std::uint32_t bytes_per_pixel);
  WGPUTextureView create_view(const Texture &tex, std::uint32_t base_mip,
  ...
  WGPUSampler create_sampler(WGPUAddressMode mode, WGPUFilterMode filter,
                             bool mips, std::uint16_t anisotropy,
                             WGPUCompareFunction compare, const char *label);
...
private:
  ...
  void upload_texture_region(const Texture &tex, std::uint32_t mip,
