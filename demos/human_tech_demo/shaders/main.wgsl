...
fn interior_color(aux: vec2<f32>, view_ts: vec3<f32>, room: vec4<f32>, seed: f32, night: f32) -> vec3<f32> {
  ...
  let floor_on = floor_h < room.w * select(0.58, 0.78, night > 0.5);
...
@fragment fn fs_main(in: VOut, @builtin(front_facing) front: bool) -> FsOut {
  ...
  if ((in.material == 4u || in.material == 5u || in.material == 6u) && N.y > 0.8) {
    let q = in.world.xz * 0.10;
    ...
    let wet=smoothstep(0.48,0.72,mix(mix(a,b,w.x),mix(c,d,w.x),w.y));
  ...
  if ((flags & FLAG_GLASS) != 0u) {
    ...
    let floor_far = hash13(vec3<f32>(17.0, cell.y + 3.3, in.aux.z * 51.3)) < m.room.w * select(0.58, 0.78, night > 0.5);
    ...
    let lit_expect = m.room.w * select(0.58, 0.78, night > 0.5) * 0.5;
    ...
    inside = inside * frame.params.x * mix(0.65, 0.9, night);
    ...
    let tint = m.misc.yzw;
    color = refl * brdf * vec3<f32>(0.72,0.90,1.0) + sun * spec_sun * shadow + inside * (1.0 - brdf) * tint;
  ...
  let density = 0.00048 * exp(-max(in.world.y, 0.0) / 440.0);
  let fog = 1.0 - exp(-distance_m * density);
  let sight = normalize(in.world - frame.camera_pos.xyz);
  let horizon = textureSampleLevel(sky_cube, cube_samp, normalize(vec3<f32>(sight.x, 0.02, sight.z)), 3.0).rgb;
  color = mix(color, horizon * frame.params2.x, fog);
