...
// Per-layer draw-purpose registry (append-only per layer). Kept global in
// v0 for simplicity; the (layer key, channel) pair is what must be unique.
enum class Channel : std::uint32_t {
  Params = 1,     // parameter block draws
  Archetype = 2,  // province archetype pick
  Lattice = 3,    // gradient-lattice base for the sanctioned cheap mixer
  Test = 0xFFFFFF,
};
