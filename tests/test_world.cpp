...
TEST_CASE("chunk manager: output independent of worker count") {
  const core::Key body = body_for(99);
  ...
  world::ChunkManager manager_a(body, planet, config_a);
  world::ChunkManager manager_b(body, planet, config_b);
...
TEST_CASE("chunk manager: closer camera yields finer chunks; eviction respects budget") {
  const core::Key body = body_for(7);
  ...
  world::ChunkManager manager(body, planet, config);
...
TEST_CASE("chunk manager: scene hash is a pure function") {
  const core::Key body = body_for(3);
  const gen::PlanetParams planet = gen::derive_planet_params(body, gen::PlanetType::Ice);
  world::ChunkManagerConfig config;
  config.worker_count = 2;
  world::ChunkManager manager_a(body, planet, config);
  world::ChunkManager manager_b(body, planet, config);
