...
// Relief at ~/2.5 of real Earth (planetary-systems spec section 4:
// vertical relief does NOT follow the 1:10 rule — exaggeration x3-5
// relative to the planet keeps horizons dramatic).
constexpr std::array<ArchetypeSpec, 5> kEarthLike = {{
    {Archetype::Flats, 3, 50, 130, -80, 80, 0.10, 0.30, 0.00, 0.10},
    {Archetype::RollingHills, 3, 200, 500, 0, 250, 0.30, 0.50, 0.10, 0.20},
    {Archetype::Alpine, 2, 1500, 3500, 500, 1200, 0.70, 1.00, 0.30, 0.60},
    {Archetype::Canyon, 2, 400, 1000, 150, 600, 0.50, 0.70, 0.70, 1.00},
    {Archetype::HighlandPlateau, 2, 250, 600, 600, 1400, 0.20, 0.40, 0.20, 0.40},
}};
constexpr std::array<ArchetypeSpec, 3> kBarren = {{
    {Archetype::RegolithPlains, 3, 60, 200, -50, 100, 0.20, 0.40, 0.00, 0.10},
    {Archetype::Cratered, 3, 250, 700, 0, 160, 0.50, 0.80, 0.40, 0.70},
    {Archetype::Highlands, 2, 500, 1400, 300, 1000, 0.50, 0.80, 0.10, 0.30},
}};
constexpr std::array<ArchetypeSpec, 3> kDesert = {{
    {Archetype::Dunes, 3, 130, 330, -30, 130, 0.30, 0.50, 0.00, 0.10},
    {Archetype::Mesas, 2, 400, 800, 160, 500, 0.40, 0.60, 0.60, 0.90},
    {Archetype::Canyonlands, 2, 500, 1100, 80, 400, 0.60, 0.80, 0.70, 1.00},
}};
constexpr std::array<ArchetypeSpec, 3> kIce = {{
    {Archetype::GlacialShield, 3, 80, 250, 0, 330, 0.10, 0.30, 0.00, 0.10},
    {Archetype::CrevasseField, 2, 330, 750, 80, 400, 0.60, 0.90, 0.50, 0.80},
    {Archetype::RidgeField, 2, 800, 2000, 330, 1000, 0.70, 1.00, 0.30, 0.50},
}};
...
ProvinceField::ProvinceField(const core::Key& body_key, const PlanetParams& planet)
    : provinces_key_(core::derive_named(body_key, name::ProvincesV1)),
      type_(planet.type),
      cells_per_face_(planet.cells_per_face) {}

ProvinceParams ProvinceField::cell_params(const CellId& cell) const {
  const core::Key cell_key = core::derive_child(provinces_key_, kind::Province, cell.face,
                                                cell.ci, cell.cj);
  const auto draw0 = core::draw_point(cell_key, channel::Params, 0, 0, 0);
  const auto draw1 = core::draw_point(cell_key, channel::Params, 1, 0, 0);
